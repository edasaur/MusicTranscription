...
while ord(wf.readframes(1)[0]) == 0:
    pass
data = wf.readframes(chunk/channels)
fmt = "%dh"%(len(data)/swidth)
print("%s frames, %s channels, %s swidth, %s fps, %s, %s fmt" % (wf.getnframes(), channels, swidth, RATE, wf.getcompname(), fmt))
...
class Notes:
    ...
    def cluster(self):
        x1 = lambda x: x[1]
        if self.all_notes[-1][0] == "r":
            self.all_notes.pop()

        to_pop = set()
        for i, note in enumerate(self.all_notes):
            if note[1] <= 1 and i != len(self.all_notes)-1 and i != 0:
                if self.all_notes[i+1][0] == self.all_notes[i-1][0]:
                    self.all_notes[i-1][1] += self.all_notes[i+1][1]
                    self.all_notes[i+1] = self.all_notes[i-1]
                    to_pop.add(i+1)
                self.all_notes[i-1][1] += 1
                to_pop.add(i)
        filtered = filter(lambda x: x[0] not in to_pop, enumerate(self.all_notes))
        self.all_notes = list(map(x1, filtered))

        MAX_GAP = 1.3
        sorted_notes = sorted(self.all_notes, key=lambda x: x[1])
        groups = [[sorted_notes[0]]]
        for x in sorted_notes:
            diff = x[1] / float(groups[-1][-1][1])
            if diff <= MAX_GAP:
                groups[-1].append(x)
                continue
            mult = 2
            while diff >= MAX_GAP * mult:
                mult *= 2
                groups.append(None)
            groups.append([x])
...
while len(data) == chunk*swidth:
    ...
    indata = np.array(wave.struct.unpack(fmt, data))

    if np.std(indata) <= 0.1:
        print(np.std(indata))
        thefreq = -1
